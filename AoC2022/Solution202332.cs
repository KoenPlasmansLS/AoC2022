namespace AoC2022
{
    public class Solution202332 : IProvideSolution
    {
        private bool[,] left;
        private bool[,] right;
        private bool[,] up;
        ...
        protected string BaseAlgorithm(string[] lines)
        {
            ...
            {
                int sum = DoOne(0, i, Dir.Right);
                max = Math.Max(max, sum);
            }
            for (var i = 0; i <= arr.GetLength(1); i++)
            {
                int sum = DoOne(arr.GetLength(0) - 1, i, Dir.Left);
                max = Math.Max(max, sum);
            }
            for (var i = 0; i <= arr.GetLength(0); i++)
            {
                int sum = DoOne(i, 0, Dir.Down);
                max = Math.Max(max, sum);
            }
            for (var i = 0; i <= arr.GetLength(0); i++)
            {
                int sum = DoOne(i, arr.GetLength(1) - 1, Dir.Up);
                max = Math.Max(max, sum);
            }
            return max.ToString();
        }

        private int DoOne(int x, int y, Dir dir)
        {
