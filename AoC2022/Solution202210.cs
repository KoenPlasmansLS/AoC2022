namespace AoC2022
{
    public class Solution202210 : IProvideSolution
    {
        ...
        protected static string BaseAlgorithm(string[] lines)
        {
            ...
            for (var j = 0; j < lines.Count(); j++)
            {
                ...
                else
                {
                    ...
                    //move 5 from 3 to 6
                    var curLst = new List<char>();
                    for(var t = 0; t < int.Parse(splt[1]); t++)
                    {
                        curLst.Add(lst[int.Parse(splt[3])-1].Pop());
                    }
                    for (var t = curLst.Count - 1; t >= 0; t--)
                    {
                        lst[int.Parse(splt[5]) - 1].Push(curLst[t]);
                    }
                }
            }
